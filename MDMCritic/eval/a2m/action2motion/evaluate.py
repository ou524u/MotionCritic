...
from .accuracy import calculate_accuracy, calculate_critic
from .fid import calculate_fid
from .diversity import calculate_diversity_multimodality


class A2MEvaluation:
    def __init__(self, device, critic_model):
    ...
    def evaluate(self, model, loaders):
        ...
        if "gt" in loaders:
            ...
            gtstats = computedfeats["batch_gt"]["stats"]
        # computing fid
        ...
        for key, loader in loaders.items():
            metric = "critic"
            mkey = f"{metric}_{key}"
            print_logs("critic", key)
            metrics[mkey] = calculate_critic(model, critic_model=self.critic_model, motion_loader=loader, device=self.device)
